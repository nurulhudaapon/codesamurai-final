...
export class EcosyncLandfillDumpingService {
     ...
     */
    create(landfilldumpingData: Entity.landfill_dumpings): Promise<Entity.landfill_dumpings> {
        // Validate 
        const validLandfillDumpingData = Schema.landfill_dumpingsSchema.parse(landfilldumpingData);

        // Commit
        return this.#client.landfill_dumpings.create({ data: validLandfillDumpingData });
    }
     ...
     */
    update(id: string, landfilldumpingData: object): Promise<Entity.landfill_dumpings | null> {
        // Validate 
        const validLandfillDumpingData = Schema.landfill_dumpingsSchema.parse(landfilldumpingData);

        // Commit
        return this.#client.landfill_dumpings.update({ where: { id }, data: validLandfillDumpingData });
    }
